...
function u_online($minutos = 15) {
	...
	global $dbconn;
	$tiem = $minutos * 60;
	$waa = mysqli_fetch_array(mysqli_query($dbconn, "SELECT COUNT(DISTINCT(id)) FROM `pjs` WHERE ultimologin>" . (time() - $tiem)));
	$online = $waa[0];
...
function infopj($datos) {
	...
	if ($tiempo < 61) {
		...
		if (strlen($server[$datos['ultimosv']]) == 0) {
			...
			if (strlen($server[$datos['ultimosv']]) == 0) {
				$server[$datos['ultimosv']] = "<b style='color:red;'>Servidor cerrado =(.</b>";
			}
		}
